...
class UpworkCausalInference:
    ...
    def __init__(self):  
        ...
        self.default_output_file_name = './analyses_output/no_outliers_iqr_causal_inference_2017_12_12_upwork_analysis_unitedstates_allskills.txt'
        
    def main(self):
        default_dataframe = self.omit_from_dataframe("default")
        default_dataframe = self.omit_outliers_iqr(default_dataframe)
        self.run_causal_inference(default_dataframe)

    def omit_outliers_iqr(self, df): 
        percentile_25 = df.bill_rate.quantile(0.25)
        percentile_75 = df.bill_rate.quantile(0.75)
        iqr = stats.iqr(df.bill_rate)
        ...
        df['outlier_iqr'] = df.apply(check_outlier_iqr, axis=1) # Add column to df indicating whether outlier or not
        df_no_outliers = df[df['outlier_iqr'] == False] 
        print "This is the length of the final df without outliers: {0}".format(len(df_no_outliers))
        return df_no_outliers # Return df with outliers removed
