...
class UpworkCausalInference:
    ...
    def __init__(self):  
        ...
        self.default_output_file_name = './analyses_output/no_outliers_2sd_causal_inference_2017_12_12_upwork_analysis_unitedstates_allskills.txt'
        
    def main(self):
        default_dataframe = self.omit_from_dataframe("default")
        default_dataframe = self.omit_outliers_2sd(default_dataframe)
        self.run_causal_inference(default_dataframe)

    def omit_outliers_2sd(self, df): 
        mean_bill_rate = np.mean(df['bill_rate'])
        sd_bill_rate = np.std(df['bill_rate'])
        df['outlier2sd'] = df['bill_rate'] >= mean_bill_rate + 2*sd_bill_rate 
        df_no_outliers = df[df['outlier2sd'] == False] 
